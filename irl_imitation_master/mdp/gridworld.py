...
class GridWorld(object):
    ...
    def get_states(self):
        ...
        for i in range(self.height):
            for j in range(self.width):
                if 'x' != self.grid[i][j] :
                    ans.append((i, j));
                    # print("not eaual", self.grid[i][j])
                # else: print("equal", self.grid[i][j]);
        return ans;
        # return [filter(
    ...
    def get_actions(self, state):
        ...
        if self.grid[state[0]][state[1]] == 'x':
            return [4]
        # else:
            print(f"grid[state[0]][state[1]]: {self.grid[state[0]][state[1]]}")
    ...
    def get_transition_states_and_probs(self, state, action):
        ...
        """
        # print("next", (state[0] + self.neighbors[action][0], state[1] + self.neighbors[action][1]));
        ...
        if self.trans_prob == 1:
            ...
            for a in range(self.n_actions):
                ...
                nei_s = (state[0] + inc[0], state[1] + inc[1])
                # print("in get_transition", self.grid[int(nei_s[0])][int(nei_s[1])])
