...
def generate_demonstrations(gw, policy, n_trajs=100, len_traj=20, rand_start=False, start_pos=[4,4], determine = True):
  ...
  trajs = []
  print("current 1Dpos", gw.pos2idx(start_pos))
  for i in range(n_trajs):
    ...
    action, next_state, reward, is_done = 0, 0, 0, 0
    if(determine == False):
      action = np.random.choice([0, 1, 2, 3, 4], p = policy[gw.pos2idx(cur_state), :])
      cur_state, action, next_state, reward, is_done = gw.step(action)
    else: 
      cur_state, action, next_state, reward, is_done = gw.step(int(policy[gw.pos2idx(cur_state)]))
    
    print("current 1Dpos_2", cur_state)
...
def main():
  ...
  print("reward before expert = normalze?", gw.grid, rewards_gt);
  trajs = generate_demonstrations(gw, policy_gt, n_trajs=N_TRAJS, len_traj=L_TRAJ, rand_start=RAND_START)
  print("one trajectory", trajs[0])
  print("nummber of expert", len(trajs))
  ...
  print("reward maxent")
  for i in range(len(rewards_maxent)):
    print(rewards_maxent[i], '', end = '') 
    if (i + 1) % 5 == 0:
        print();


  # print("rewrad", rewards_maxent);
  # print(f"grid: {gw.grid}")
  # print(f"now feat_map: {feat_map}");
  # print(f"now p_a: {P_a}");
  # print(f"now trajs: {trajs}");
